...
class SettingController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','general'),
				'users'=>array('@'),
			),
    ...
    public function actionGeneral(){
        ...
        $criteria->select = array('distinct category');
        $criteria->condition = "category <> 'Payment' AND category <> 'Credit'";
        $models = Setting::model()->findAll($criteria);
        if(isset($_POST['Setting'])){
            foreach($_POST['Setting'] as $key => $value){
                $command = Yii::app()->db->createCommand();
                $command->update('setting', array('value'=>$value), 'name=:name', array(':name'=>$key));
            }
